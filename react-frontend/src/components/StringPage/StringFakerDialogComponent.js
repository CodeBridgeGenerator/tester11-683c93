...
import stringFakerFactory from "./stringFakerFactory";
...
const StringFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = stringFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("string").create(elem));
...
export default connect(mapState, mapDispatch)(StringFakerDialogComponent);
